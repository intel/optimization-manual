...
static void BM_g2s_vpgatherdd(benchmark::State &state)
{
	...
	for (auto _ : state) {
		g2s_vpgatherdd(len, aos, soa_imaginary, soa_real);
	}
...
static void BM_g2s_vpermi2d(benchmark::State &state)
{
	if (!supports_avx512_skx()) {
		state.SkipWithError("AVX-512 not supported, skipping test");
		return;
	...
	for (auto _ : state) {
		g2s_vpermi2d(len, aos, soa_imaginary, soa_real);
	}
	state.SetBytesProcessed(int64_t(state.iterations()) * int64_t(len) *
				int64_t(sizeof(*aos)));
