...
static unsigned char b[128] __attribute__((aligned(64)));
...
TEST(avx512_22, lookup128_novbmi)
{
	...
	ASSERT_EQ(lookup128_novbmi_check(a, b, out, MAX_SIZE), true);
	for (size_t i = 0; i < MAX_SIZE; i++) {
		ASSERT_EQ(c_out[i], out[i]);
	}
	ASSERT_EQ(lookup128_novbmi_check(NULL, b, out, MAX_SIZE), false);
	ASSERT_EQ(lookup128_novbmi_check(a, NULL, out, MAX_SIZE), false);
	ASSERT_EQ(lookup128_novbmi_check(a, b, NULL, MAX_SIZE), false);
	ASSERT_EQ(lookup128_novbmi_check(a, b, out, MAX_SIZE - 16), false);
}

TEST(avx512_22, lookup128_vbmi)
{
	...
	init_sources();
	lookup(a, c_out, b, MAX_SIZE);
	ASSERT_EQ(lookup128_vbmi_check(a, b, out, MAX_SIZE), true);
	for (size_t i = 0; i < MAX_SIZE; i++) {
		ASSERT_EQ(c_out[i], out[i]);
	}
	ASSERT_EQ(lookup128_vbmi_check(NULL, b, out, MAX_SIZE), false);
	ASSERT_EQ(lookup128_vbmi_check(a, NULL, out, MAX_SIZE), false);
	ASSERT_EQ(lookup128_vbmi_check(a, b, NULL, MAX_SIZE), false);
	ASSERT_EQ(lookup128_vbmi_check(a, b, out, MAX_SIZE - 32), false);
}
