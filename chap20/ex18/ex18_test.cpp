...
#include "gemm/amx_interleaved_gemm_relu_ass.hpp"
#include "amx_int8_uint8_test_utils.hpp"
#include "optimisation_common.h"

// clang-format on

TEST(amx_18, amx_interleaved_gemm_int8_relu)
{
	if (!supports_amx())
	...
	for (size_t m = 0; m < M; m++)
		for (size_t n = 0; n < N; n++) {
			ASSERT_EQ(C_tmp_mem_out_expected[m][n],
				  C_tmp_mem_out[m][n]);
			ASSERT_EQ(C_mem_expected[m][n], C_mem[m][n]);
		}
}
#else
TEST(amx_ex18, amx_interleaved_gemm_int8_relu)
{
