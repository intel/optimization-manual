...
TEST(avx_17, three_tap_avx)
{
	init_sources();
	ASSERT_EQ(three_tap_avx_check(a, coeff, out, MAX_SIZE - 2), true);
	for (size_t i = 0; i < MAX_SIZE - 2; i++) {
		float expected =
		    a[i] * coeff[0] + a[i + 1] * coeff[1] + a[i + 2] * coeff[2];
		ASSERT_FLOAT_EQ(expected, out[i]);
	}
	ASSERT_EQ(three_tap_avx_check(a, coeff, out, MAX_SIZE), false);
	ASSERT_EQ(three_tap_avx_check(NULL, coeff, out, MAX_SIZE - 2), false);
	ASSERT_EQ(three_tap_avx_check(a, NULL, out, MAX_SIZE - 2), false);
	ASSERT_EQ(three_tap_avx_check(a, coeff, NULL, MAX_SIZE - 2), false);
	ASSERT_EQ(three_tap_avx_check(a + 1, coeff, out, MAX_SIZE - 2), false);
	ASSERT_EQ(three_tap_avx_check(a, coeff, out + 1, MAX_SIZE - 2), false);
}
