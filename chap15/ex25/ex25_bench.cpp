...
static void BM_sqrtps_divps_sse(benchmark::State &state)
{
...
static void BM_sqrtps_vdivps_avx(benchmark::State &state)
{
	int len = state.range(0);
...
BENCHMARK(BM_sqrtps_divps_sse)
    ...
    ->Arg(1 << 16)
    ->Arg(1 << 18);
BENCHMARK(BM_sqrtps_vdivps_avx)
    ->Arg(1 << 6)
    ->Arg(1 << 8)
    ->Arg(1 << 10)
    ->Arg(1 << 12)
