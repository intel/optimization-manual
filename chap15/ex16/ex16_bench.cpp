...
static void BM_three_tap_sse(benchmark::State &state)
{
	int len = state.range(0);
	float coeff[3];
	float *a = (float *)_mm_malloc(len * sizeof(float), 16);
	float *out = (float *)_mm_malloc(len * sizeof(float), 16);

	init_sources(a, coeff, len);

	for (auto _ : state) {
		three_tap_sse(a, coeff, out, len - 2);
	}
