...
static void BM_median_sse(benchmark::State &state)
{
...
static void BM_median_avx_overlap(benchmark::State &state)
{
	int len = state.range(0);
...
static void BM_median_avx_vperm(benchmark::State &state)
{
	int len = state.range(0);
	float *x = (float *)_mm_malloc(len * sizeof(float), 32);
	float *y = (float *)_mm_malloc(len * sizeof(float), 32);
...
BENCHMARK(BM_median_sse)
    ...
    ->Arg(1 << 16)
    ->Arg(1 << 18);
BENCHMARK(BM_median_avx_overlap)
    ->Arg(1 << 6)
    ->Arg(1 << 8)
    ->Arg(1 << 10)
    ->Arg(1 << 12)
